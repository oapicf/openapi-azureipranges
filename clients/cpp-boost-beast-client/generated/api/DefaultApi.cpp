...
DefaultApiException::DefaultApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status DefaultApiException::getStatus() const
{
    return m_status;
}
...
DefaultApi::getAzureIpRangesServiceTagsPublicCloud(
    const std::string& version) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ServiceTags_Public_{version}.json";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "version", version);

    std::string responseContentType = "application/json";
    static const std::vector<std::string> acceptTypes{ "application/json","application/octet-stream", };
    ...
    std::shared_ptr<Change> deserializedResponse = std::make_shared<Change>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Change>>::deserialize(
    ...
    throw DefaultApiException(statusCode, "Unexpected HTTP status code");
}


std::string DefaultApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void DefaultApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void DefaultApi::handleStdException(const std::exception&) {
    throw;
}

void DefaultApi::handleUncaughtException() {
    throw;
}
