...
- Build date: 2026-08-24T12:18:14.472046739Z[Etc/UTC]
...
```
cargo run --example openapi_client-client GetAzureIpRangesServiceTagsPublicCloud
```
...
```toml
[dependencies]
openapi_client = { version = "1.0.1-pre.0", default-features = false, features = ["client", "server"] }
```

**For server with callbacks that need HTTPS:**
```toml
[dependencies]
openapi_client = { version = "1.0.1-pre.0", features = ["server", "client-tls"] }
```
